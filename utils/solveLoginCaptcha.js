...
async function solve(page) {
  try {
    await page.waitForFunction(() => {
      const iframe = document.querySelector('iframe[src*="api2/anchor"]'); // 1 iframe exist
      // const iframe = document.querySelectorAll('iframe[src*="api2/anchor"]')[1]; // multiple iframes exit
      ...
      console.log(
        "test",
        iframe.contentWindow.document.querySelector("#recaptcha-anchor")
      );
    ...
    await checkbox.click({ delay: rdn(1000, 4000) });
    ...
    await audioButton.click({ delay: rdn(2000, 4000) });
